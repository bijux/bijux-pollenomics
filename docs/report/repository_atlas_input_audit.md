...
| RAÄ archaeology context | `contextual_domain` | `data/raa/normalized/sweden_archaeology_layer.json` | `publication_status` refused_not_publication_ready, `published_site_count` None, `density_cell_count` None, `reason_codes` ['missing_raw_inventory', 'missing_raw_summary', 'missing_scientific_review'] | RAÄ is explicitly Sweden-scoped and should never be mistaken for Nordic-wide archaeology coverage. |
...
| Animal aDNA publication surface | `contextual_domain` | `docs/report/animal_sample_database_review.json` | `published_point_count` 233, `unresolved_row_count` 0 | Animal aDNA is still a partial recovery program whose public map surface depends on sample-owned support reviews and release gates. |
