...
The current public report tree ships `249` mapped non-human animal atlas layer rows and `2` country-resolved animal output hits across the species table above. Those counts still need to be read beside blocked and unresolved sample totals rather than as a standalone readiness claim.
