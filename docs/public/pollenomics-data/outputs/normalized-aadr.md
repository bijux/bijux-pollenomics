...
This page keeps one point clear: AADR is the strongest direct bridge between
one upstream version and visible publication change. Readers should be able to
see that version identity all the way into reports and atlas layers.
...
release and not genotype analysis artifacts. Their role is to make the
checked-in sample layer reviewable and publishable.

## First Proof Check

- inspect `data/aadr/v66/`
- inspect the country bundle summaries under `docs/report/*/*_aadr_v66_summary.json`
- compare with [AADR](../sources/aadr.md) when the question is about upstream scope rather than checked-in outputs
