...
def resolve_homo_sapiens_schema(fieldnames: Sequence[str]) -> dict[str, str | None]:
    """Map expected AADR metadata fields to raw anno column names."""
    return {
...
def find_column(fieldnames: Sequence[str], *prefixes: str) -> str:
    """Find a column by exact name or stable prefix."""
    ...
    for prefix in prefixes:
        ...
        for field in fieldnames:
            if field.casefold().startswith(prefix_key):
                return field
    raise ValueError(f"Could not find any of {prefixes!r} in anno columns")
...
def sample_time_interval(
    ...
    """Derive one AADR BP interval from mean and standard deviation columns."""
...
def sample_time_label(row: Mapping[str, str], schema: Mapping[str, str | None]) -> str:
    """Return the best available human-readable time label."""
...
def schema_value(
    ...
    """Read one row value via a schema key while handling optional columns."""
