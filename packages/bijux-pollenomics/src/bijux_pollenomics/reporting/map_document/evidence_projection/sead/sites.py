...
from ..io import _mapping, _numeric_text_key, _read_json_object, _required_text
...
def index_sites(
    ...
    admitted_site_ids = _admitted_site_ids(bundle)
    ...
    feature_site_ids, feature_count = _bind_features(layers, sites)
    return SiteIndex(
        rows_by_id=sites,
...
def _admitted_site_ids(bundle: EvidenceBundle) -> set[str]:
    ...
    return {str(row.get("site_id")) for row in cast(list[Mapping[str, object]], rows)}
