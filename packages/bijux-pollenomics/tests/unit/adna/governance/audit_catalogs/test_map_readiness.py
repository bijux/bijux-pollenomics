...
def test_map_readiness_reconciles_point_ready_and_unpublished_counts(
    ...
    assert readiness["totals"]["direct_coordinate_backed"] == 300
    assert readiness["totals"]["indirectly_geocoded"] == 4
    assert readiness["totals"]["coordinate_provenance_mappable_count"] == 304
    assert readiness["totals"]["coordinate_provenance_row_count"] == 308
    assert readiness["totals"]["publication_candidate_count"] == 170
    ...
    assert readiness["totals"]["unresolved_sample_count"] == 402
    ...
    assert Counter(refusal_lookup.values()) == {
        ...
        "no_admitted_sample_backed_locality_candidate": 3,
        "chronology_not_supported_for_atlas_publication": 3,
    ...
    assert (
        refusal_lookup[("PRJEB75467", "Uralsk")]
        == "chronology_not_supported_for_atlas_publication"
    )
    ...
    assert cattle_row["direct_coordinate_backed"] == 24
    assert cattle_row["publication_candidate_count"] == 22
    assert cattle_row["not_materialized_count"] == 2
