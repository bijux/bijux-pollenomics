...
class AdnaCurationUnitTests(unittest.TestCase):
    def test_species_curation_manifest_builds_paper_pinned_core_manifest_for_horse(
        ...
        self.assertTrue(
            any(
                ...
                for row in manifest.curated_projects
            )
        )
        self.assertTrue(
            any(
                row.project_accession == "PRJEB56293"
                for row in manifest.pending_projects
            )
        )
        self.assertTrue(
            any(
    ...
    def test_species_curation_manifest_builds_core_and_comparator_manifests_for_new_species(
        ...
        self.assertTrue(
            any(row.project_accession == "SRS1407451" for row in dog.curated_projects)
        )
        ...
        self.assertTrue(
            any(
                ...
                for row in reindeer.pending_projects
            )
        )
    ...
    def test_domestication_coverage_report_marks_strong_thin_and_current_core_rows(
        ...
        self.assertTrue(
            all(row.curation_class in ADNA_CURATION_CLASSES for row in report.rows)
        )
        self.assertTrue(
            all(row.coverage_posture in ADNA_COVERAGE_POSTURES for row in report.rows)
        )
        horse = next(
            row for row in report.rows if row.species_latin_name == "Equus caballus"
        )
        chicken = next(
            row
            for row in report.rows
            if row.species_latin_name == "Gallus gallus domesticus"
        )
        cattle = next(
            row for row in report.rows if row.species_latin_name == "Bos taurus"
        )
