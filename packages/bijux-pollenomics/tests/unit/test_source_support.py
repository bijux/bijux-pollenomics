...
from bijux_pollenomics.collection.catalog.support import build_source_support_matrix
...
class SourceSupportUnitTests(unittest.TestCase):
    def test_source_support_matrix_reports_status_and_coverage(self) -> None:
        ...
        svar_row = [row for row in matrix if row.source == "svar"][0]
        self.assertEqual(svar_row.country_coverage, ("Sweden",))
