...
SECURITY_PIP_AUDIT_ENV := VIRTUAL_ENV= PIPAPI_PYTHON_LOCATION="$(VENV_PYTHON)"
...
security-audit:
	...
	JSON_RC=0; \
	TEXT_RC=0; \
	$(SECURITY_PYTHON_ENV) $(SECURITY_PIP_AUDIT_ENV) $(PIP_AUDIT) $(SECURITY_IGNORE_FLAGS) $(PIP_AUDIT_CONSOLE_FLAGS) $(PIP_AUDIT_INPUTS) \
	...
	if [ -n "$(strip $(SECURITY_PIP_AUDIT_TEXT_COMMAND))" ]; then \
	  ...
	  { set +e; $(SECURITY_PYTHON_ENV) $(SECURITY_PIP_AUDIT_ENV) $(PIP_AUDIT) $(SECURITY_IGNORE_FLAGS) $(PIP_AUDIT_CONSOLE_FLAGS) $(PIP_AUDIT_INPUTS); RC=$$?; echo $$RC > "$$RC_FILE"; exit 0; } 2>&1 | tee "$(PIPA_TXT)"; \
	  TEXT_RC=$$(cat "$$RC_FILE"); \
	  rm -f "$$RC_FILE"; \
	fi; \
	cat "$(PIPA_TXT)"; \
